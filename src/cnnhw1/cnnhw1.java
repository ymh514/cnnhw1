...
public class cnnhw1 extends JFrame { // 目前這個程式碼主要先測試感知機1
	...
	private static void genarateframe(ArrayList<float[]> array_input, float[] initial, float threshold, float liney1,
			float liney2, int framesizex, int framesizey) {
	...
	private static void printthresholdandinitial(float threshold, float[] initial) {
		...
		System.out.printf("%f\n", threshold);// use c language to print prevant
												// loss ....??
		for (int w = 0; w < initial.length; w++) {
			System.out.printf("%f\n", initial[w]);// use c language to print
													// prevant loss ....??
		}
	}
	...
	private static String LoadFile() {
		...
		fc.setCurrentDirectory(new java.io.File("E:\\1041\\cnn\\HW1\\dataset"));
	...
	private static float sumall(ArrayList<float[]> array, float[] initial, float caltemp, float threshold, float sum,
			int x0, int xn) {
		for (int w = 0; w < (array.get(xn).length - 1); w++) {// 要用的只有前兩個
																// 最後一個是desire
	...
	private static int testCheck(float[] initial, float threshold, ArrayList<float[]> array_test) {
		...
		for (int i = 0; i < testamount; i++) {
			float sum = 0f;
			for (int w = 0; w < (array_test.get(i).length - 1); w++) {// 要用的只有前兩個
																		// 最後一個是desire
			...
			if (judge == array_test.get(i)[array_test.get(i).length - 1]) {
				testcorrect++;
			}
		}
		return testcorrect;
	...
	/*
	 * 1.load file & use split then trans to float wiz token2,then store into
	 * array 2.class defination 3.copy array_input to array_temp with random,
	 * then separate 2/3 as train dataset 1/3 as test set 4.get initial
	 * information 5.do calculate & set some flag 6.genarate line equation
	 * 7.check correct ratio(call function in printCorrctRatio) then print
	 * correctratio 8.generate gui
	 */
	public static void main(String[] args) throws IOException {
		/*
		 * 1.load file & use split then trans to float wiz token2,then store
		 * into array
		 */
		...
		/*
		 * 2.class defination 先用一個基準值 如果>=1 那就直接用他設為1 其他不同設為-1 同理 如果一開始拿到0or-1
		 */
		classify(array_input);// call function to classify

		/*
		 * 3.copy array_input to array_temp with random, then separate 2/3 as
		 * train dataset 1/3 as test set
		 */
		putInputToTemp(array_input);// copy to temp with random

		separateTemp(array_temp);// separate to train and test set,set 2/3 as
		...
		Object[] message = { "Looptimes (eg.50)", usrlooptimes, "Studyrate (eg.0.8)", usrstudyrate,
				"Threshold (eg.-1.0)", usrthreshold };
		...
		whileloop: while (looptimes != 0) {
			float sum = 0f;// db2 sum is a register and you have to reset to
							// zero.
			sum = sumall(array_train, initial, caltemp, threshold, sum, x0, xn);// call
																				// function
																				// to
																				// sum
																				// all
			...
			if (judge != array_train.get(xn)[array_train.get(xn).length - 1] && judge >= 0) {
				for (int w = 0; w < (array_train.get(xn).length - 1); w++) {// 要用的只有前兩個
																			// 最後一個是desire
		...
		genarateframe(array_train, initial, threshold, liney1, liney2, framesizex, framesizey);// call
																								// function
																								// to
																								// genarate
																								// frame
